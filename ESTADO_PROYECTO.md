...
6. **`/envases`**: catálogo de envases metálicos completo y funcional, en **2 niveles** (no 3): `/envases` (hub con carruseles horizontales "estilo Netflix", una fila por categoría) y `/envases/producto/[producto]` (ficha de detalle). No tiene página intermedia de categoría — se eliminó en un refactor posterior al ver que no aportaba valor con el patrón de carrusel. Datos reales transcritos de `catalogo_latas_asiaven.md`. Sin gaps conocidos ahí.
